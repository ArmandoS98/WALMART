...
function template(product){ 
      return `<tr class="text-center" data-id="${product.docid}">
              <td><img src="${product.image}" class="card-img-top img-fluid img-thumbnail" alt="..." style="width:6rem"></td>
              <td>${product.descripcion}</td>
              <td>${product.precio}</td>
              <td><input type="number" class="cantidad form-control text-center" min="0" value="${product.cantidad}"></td>
...
function myCart() {
  var storedProducts =JSON.parse(localStorage.getItem("my_products"));
...
function addProduct(id,number){
  var storedProducts =JSON.parse(localStorage.getItem("my_products"));
  existProduct = storedProducts.findIndex(
